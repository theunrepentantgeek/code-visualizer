...
- `Builder.Build` constructs it, `ResolveOptions` turns raw CLI/config strings into position and orientation, `ReserveSpace` sizes it, and `RenderInto` decomposes it into canvas primitives ([legend.go#L41](legend.go#L41), [legend.go#L13](legend.go#L13), [config.go#L70](config.go#L70), [render.go#L16](render.go#L16)).
...
- Reserve legend space before laying out content, using the same config that will later be rendered ([config.go#L67](config.go#L67), [../treemap/stages.go#L76](../treemap/stages.go#L76)).
