...
func TestLayoutNodeCountMatchesBuckets(t *testing.T) {
	...
	for _, n := range []int{1, 10, 24, 28, 50, 100} {
		...
		layout := Layout(buckets, 1920, 1920, Hourly)
		nodes := layout.Nodes
...
func TestLayoutZeroBucketsReturnsEmpty(t *testing.T) {
	...
	layout := Layout(nil, 1920, 1920, Hourly)
	nodes := layout.Nodes
	g.Expect(nodes).To(BeEmpty())

	layout = Layout([]TimeBucket{}, 1920, 1920, Daily)
	nodes = layout.Nodes
	g.Expect(nodes).To(BeEmpty())
}
...
func TestLayoutSingleBucket(t *testing.T) {
	...
	g.Expect(n.Geometry.Radius).To(BeNumerically(">", 0))
}
...
func TestLayoutClockwiseFromNorth(t *testing.T) {
	...
	g.Expect(nodes[0].Geometry.Center.Y).To(BeNumerically("<", cy), "first node should be above centre")
	g.Expect(nodes[0].Geometry.Center.X).To(BeNumerically("~", cx, 1.0), "first node should be near centre X")
}
...
func TestLayoutPositionsWithinCanvas(t *testing.T) {
	...
	for i, n := range nodes {
		g.Expect(n.Geometry.Center.X).To(BeNumerically(">=", 0), "node %d X should be >= 0", i)
		g.Expect(n.Geometry.Center.X).To(BeNumerically("<=", 1920), "node %d X should be <= width", i)
		g.Expect(n.Geometry.Center.Y).To(BeNumerically(">=", 0), "node %d Y should be >= 0", i)
		g.Expect(n.Geometry.Center.Y).To(BeNumerically("<=", 1080), "node %d Y should be <= height", i)
	}
}
...
func TestLayoutCentreOfSpiral(t *testing.T) {
	...
	g.Expect(nodes[0].Geometry.Center.X).To(BeNumerically("~", cx, 1.0),
		"first node X should be at canvas centre X")
	g.Expect(nodes[0].Geometry.Center.Y).To(BeNumerically("<", cy),
		"first node Y should be above canvas centre")

	// Average of all X positions should be roughly centred.
	var sumX, sumY float64
	for _, n := range nodes {
		sumX += n.Geometry.Center.X
		sumY += n.Geometry.Center.Y
	}
...
func TestLayoutScalesWithCanvasSize(t *testing.T) {
	...
	smallLayout := Layout(buckets, 800, 800, Hourly)
	largeLayout := Layout(buckets, 1600, 1600, Hourly)
...
func TestLayoutManyLapsNoOverlap(t *testing.T) {
	...
	for i := range len(nodes) - spotsPerLap {
		...
		minGap := current.Geometry.Radius + nextLap.Geometry.Radius
...
func TestLayoutFitsWithinCanvasIncludingDisc(t *testing.T) {
	...
	for i, n := range nodes {
		g.Expect(n.Geometry.Center.X-n.Geometry.Radius).To(BeNumerically(">=", 0),
			"node %d left edge should be >= 0", i)
		g.Expect(n.Geometry.Center.X+n.Geometry.Radius).To(BeNumerically("<=", 1920),
			"node %d right edge should be <= width", i)
		g.Expect(n.Geometry.Center.Y-n.Geometry.Radius).To(BeNumerically(">=", 0),
			"node %d top edge should be >= 0", i)
		g.Expect(n.Geometry.Center.Y+n.Geometry.Radius).To(BeNumerically("<=", 1080),
...
func TestLayoutSpiralParamsConsistentWithNodes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			layout := Layout(buckets, tc.width, tc.height, tc.resolution)
			...
			for i, node := range layout.Nodes {
				...
				g.Expect(node.Geometry.Center.X).To(
				...
				g.Expect(node.Geometry.Center.Y).To(
	...
	t.Run("zero buckets", func(t *testing.T) {
		...
		emptyLayout := Layout(nil, 1920, 1920, Hourly)
